...
const Footer: FC = () => {
  ...
  return (
    <footer className="relative w-full px-6 py-12 mt-20 bg-background text-foreground border-t border-border transition-colors">
      {/* Top gradient bar — blue to violet */}
      <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-blue-500 via-violet-500 to-purple-600" />
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <motion.div className="space-y-4" variants={childVariants}>
            <h1 title="Jahannivas" className="text-2xl font-extrabold bg-gradient-to-r from-blue-500 via-violet-500 to-purple-600 bg-clip-text text-transparent">
              Jahannivas
            </h1>
              ...
              {quickLinks.map((item) => (
                <li key={item.name}>
                  <TrackableElement elementId={`footer-nav-${item.name.toLowerCase()}`} elementText={`Footer: ${item.name}`}>
                    <a
                      href={item.href}
                      className="hover:text-violet-500 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-violet-500"
              ...
              {socialLinks.map((link) => (
                <TrackableContact key={link.label} method={`footer-${link.label.toLowerCase()}`}>
                  <motion.a
                    ...
                    className="flex items-center justify-center w-10 h-10 bg-muted rounded-full hover:bg-gradient-to-r hover:from-blue-600 hover:to-violet-600 hover:text-white transition-all duration-300"
